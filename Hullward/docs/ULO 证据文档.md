...
> 版本：v1.1（含 sprint 2：任务制循环）｜ 日期：2026-09-17 ｜ 用途：SIT771 "Something Awesome" 7.4H 任务最终证据
...
- `dotnet build` 全程 0 error 0 warning；`dotnet test` 81/81 通过

### 证据 3：Git 工作流

23 个功能提交，每个提交单一时序逻辑（scaffold → 设计 → 域层 → 表现层 → 修复 → 发布），见 `git log --oneline`。
...
- 域层状态（位置/耐久/背包/存档/星图/章节）与表现层渲染完全隔离
- 存档多文件制：`SaveService` 封装目录/文件操作，调用方只面对 `Load(name)/Save(name)/List()/Delete()`
...
- 随机逻辑注入 `Random` 种子保证确定性；边界情况（空列表/槽满/护盾封顶/最低品质/存档重名/章节越界）均有用例
- 回归价值实证：`AutoEquip_ArmorModule_IncreasesShield` 捕获"白装永不装配"真实 bug
...
- 系统实现顺序表（迭代 1-4 映射）+ 验收标准
- 战斗结算公式、掉落品质权重表、星域难度缩放公式
- UI 流程（v1.1 增补）：主菜单 → 命名/选档 → 星图 → 战斗 → 结算 → 星图重随机（对应 LD UI 规格 v0.2 的 mermaid 总览）
...
| 调试定位问题能力 | `工程日志.md` 7 条"现象→根因→修复"记录 |
| 抽象/继承/多态运用 | `src/Domain/Ships/ShipBase.cs`、`src/Domain/Enemies/EnemyShip.cs`、`src/Domain/Combat/ActiveSkill.cs`、`src/Domain/Modules/IShipModule.cs`、`src/Domain/WorldGen/ChapterCatalog.cs` |
| 实现并测试 | `tests/Hullward.Tests/` 10 个测试类 81 用例全绿 |
| 设计表达能力 | `docs/设计文档.md`（类图/场景树/表格/公式/UI 流程） |
| 完整可交付 | release exe `build/Hullward.exe`（含 .NET 运行时，启动验证通过） |
| 可复现 | `git log` 23 提交；README 运行/测试命令 |
